...
const uint8_t msgSetupNewTagSingleTrackMode = 106;  // 07
const uint8_t msgSetupNewTagTrack = 109;            // 10
...
void playNextTrack(uint16_t globalTrack, bool directionForward, bool isInteractive) {
  ...
  //single track mode: stop after playing one track
  if (nfcTag.playbackMode == 4) {
    //playTrack = nfcTag.trackInFolder;
    //Serial.print(F("mp3 | single track mode -> folder "));
    //Serial.print(nfcTag.assignedFolder);
    //Serial.print(F(" -> track "));
    //Serial.print(playTrack);
    //Serial.print(F(" of "));
    //Serial.println(folderTrackCount);
    //mp3.playFolderTrack(nfcTag.assignedFolder, playTrack);
    Serial.println(F("mp3 | single track mode: STOP"));
	mp3.stop();
	playTrack = 0;
    ...
    if (mifareStatus != MFRC522::STATUS_OK) {
      ...
      if (mifareStatus != MFRC522::STATUS_OK) {
        ...
        if (tempCookie != 0) {
		  ...
		  nfcTag.trackInFolder = mifareData[7];
        } 
...
void setup() {
  ...
  Serial.println(F("sys | BY Uli König"));
  Serial.println(F("---------------------"));
...
//pick Track from a folder
uint8_t pickSingleTrack(uint8_t folder){
	bool setTrackInFolder = false;
	Serial.println(F("sys |   pick track"));
    mp3.playMp3FolderTrack(msgSetupNewTagTrack);
	do {
        ...
        while (!setTrackInFolder);
		return nfcTag.trackInFolder;
}
      ...
      if (nfcTag.cookie == 322417479) {
        ...
        switch (nfcTag.playbackMode) {
          ...
          case 3:
            ...
            break;
          case 4:
            Serial.println(F("single track mode"));
            break;
          default:
            break;
        }
        ...
        switch (nfcTag.playbackMode) {
          ...
          case 4:
            playTrack = nfcTag.trackInFolder;
            Serial.print(F("mp3 | single track mode "));
            Serial.print(folderTrackCount);
            Serial.print(F(" tracks in folder "));
            Serial.println(nfcTag.assignedFolder);
            Serial.print(F("mp3 | single track mode -> folder "));
      ...
      // # nfc tag does not have our magic cookie 0x1337 0xb347 on it (0), start setup to configure this nfc tag
        ...
        do {
          ...
          // button 2 (right) single push or ir remote up / right: next playback mode
            ...
            switch (nfcTag.playbackMode) {
              ...
              case 4:
                Serial.println(F("single track mode"));
                mp3.playMp3FolderTrack(msgSetupNewTagSingleTrackMode);
                break;
              default:
                break;
            }
        ...
        switch (nfcTag.playbackMode) {
		  ...
		  case 3:
            ...
            break;
		  case 4:
            Serial.print(F("single track mode"));
			nfcTag.trackInFolder = pickSingleTrack(nfcTag.assignedFolder);
            break;
          default:
            break;
        }
        ...
        uint8_t bytesToWrite[] = { 0x13, 0x37, 0xb3, 0x47,            // 0x1337 0xb347 magic cookie to identify our nfc tags
                                   ...
                                   nfcTag.trackInFolder,              // track in folder for single track mode
